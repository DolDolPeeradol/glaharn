...
import { Person, Item, PersonSummary, PaymentMethod, Comment } from "@/types";
...
export default function PartyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { isAdmin, loading: accessLoading } = usePartyAccess(partyId);
  ...
  useEffect(() => {
    ...
    const unsubPeople = subscribeToPeople(partyId, setPeople);
    const unsubItems = subscribeToItems(partyId, setItems);
  ...
  // Subscribe to comments when person detail is open
  useEffect(() => {
    if (!partyId || !selectedPerson) {
      setComments([]);
      return;
    }

    const unsubComments = subscribeToComments(partyId, selectedPerson.id, setComments);

    return () => {
      unsubComments();
    };
  }, [partyId, selectedPerson]);

  // Generate share link
  useEffect(() => {
    if (typeof window !== "undefined") {
      setShareLink(`${window.location.origin}/party/${partyId}`);
    }
  }, [partyId]);
  ...
  const handleAddPerson = async () => {
    if (personName.trim()) {
      try {
        await addPerson(partyId, { name: personName.trim() } as Person);
        setPersonName("");
  ...
  const handleEditPerson = async (id: string, newName: string) => {
    try {
      await updatePerson(partyId, id, { name: newName });
    } catch (error) {
  ...
  const handleDeletePerson = async (id: string) => {
    try {
      await deletePerson(partyId, id);
      // อัพเดท items ที่เกี่ยวข้อง
      for (const item of items) {
        if (item.paidBy === id) {
          await deleteItem(partyId, item.id);
        } else {
          const newSharedBy = calculateSharedBy(
            people.filter((p) => p.id !== id),
            item.id
          );
          if (newSharedBy.length > 0) {
            await updateItem(partyId, item.id, { sharedBy: newSharedBy });
          } else {
            await deleteItem(partyId, item.id);
          }
        }
  ...
  const handleEditItem = async (id: string, updates: Partial<Item>) => {
    try {
      await updateItem(partyId, id, updates);
    } catch (error) {
  ...
  const handleUpdatePaymentMethod = async (personId: string, paymentMethod: PaymentMethod) => {
    try {
      await updatePerson(partyId, personId, { paymentMethod });
    } catch (error) {
      console.error("Error updating payment method:", error);
    }
  };

  const handleToggleFoodItem = async (personId: string, itemId: string, isExcluded: boolean) => {
    try {
      if (isExcluded) {
        // เพิ่มใน excludedItems
        await updatePerson(partyId, personId, {
          excludedItems: [...(people.find(p => p.id === personId)?.excludedItems || []), itemId]
        });
      } else {
        // ลบออกจาก excludedItems
        const currentExcluded = people.find(p => p.id === personId)?.excludedItems || [];
        await updatePerson(partyId, personId, {
          excludedItems: currentExcluded.filter(id => id !== itemId)
        });
      }

      // อัพเดท sharedBy ของ item
      const item = items.find(i => i.id === itemId);
      if (item) {
        const newSharedBy = people
          .filter(p => {
            if (p.id === personId) {
              return !isExcluded; // ถ้า exclude ก็ไม่อยู่ใน sharedBy
            }
            return !(p.excludedItems || []).includes(itemId);
          })
          .map(p => p.id);

        await updateItem(partyId, itemId, { sharedBy: newSharedBy });
      }
    } catch (error) {
  ...
  const handleAddComment = async (text: string, authorName: string) => {
    ...
    try {
      await addComment(partyId, selectedPerson.id, text, authorName);
    } catch (error) {
  ...
  const handleDeleteComment = async (commentId: string) => {
    if (!selectedPerson) return;
    try {
      await deleteComment(partyId, selectedPerson.id, commentId);
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50">
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-4">
            <div className="text-center lg:text-left">
              ...
              <p className="text-emerald-50 text-xs sm:text-sm md:text-base font-medium">
                {isAdmin ? "โหมด Admin - คุณสามารถแก้ไขข้อมูลได้" : "โหมด Guest - ดูข้อมูลและคอมเมนต์"}
              </p>
            </div>
          ...
          {isAdmin && (
            <div className="mt-4 flex justify-center lg:justify-start">
              <button
                ...
                className="px-4 py-2 bg-white/20 backdrop-blur-md text-white rounded-xl hover:bg-white/30 transition-all border-2 border-white/40 font-semibold"
              >
                📋 คัดลอกลิงก์แชร์
              </button>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl p-4 sm:p-6 mb-4 sm:mb-6 border-2 border-emerald-100">
          ...
          <PeopleChips
            people={people}
            onEdit={isAdmin ? handleEditPerson : undefined}
            onDelete={isAdmin ? handleDeletePerson : undefined}
            onAdd={isAdmin ? () => setIsPersonModalOpen(true) : undefined}
